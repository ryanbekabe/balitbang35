...
//include "../functions/fungsi_konversiuser.php";
	$sql = "select * from t_chat where (t_chat.to = '".mysql_real_escape_string($_SESSION['username'])."' AND recd = 0) order by id ASC";
...
EOD;
	...
	foreach ($_SESSION['openChatBoxes'] as $chatbox => $time) {
		if (!isset($_SESSION['tsChatBoxes'][$chatbox])) {
			...
			//$selisih = ambilselisih(strtotime($time), time());
			$message = "Dikirim $time";
			if ($now > 180) {
				$items .= <<<EOD
{
"s": "2",
...
EOD;
	...
	$sql = "update t_chat set recd = 1 where t_chat.to = '".mysql_real_escape_string($_SESSION['username'])."' and recd = 0";
	$query = mysql_query($sql);
